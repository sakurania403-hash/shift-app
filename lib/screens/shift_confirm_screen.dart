...
class _ShiftTimelineScreenState extends State<ShiftTimelineScreen> {
  ...
  Future<void> _loadData() async {
    ...
    final isHoliday =
        await StoreSettingsService.isHoliday(widget.date, holidays);
  ...
  String get _dateStr => widget.date.toIso8601String().substring(0, 10);
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title: Text('${fmt.format(widget.date)} - $name'),
  ...
  Widget _buildStaffTimebar({
    ...
    final hasRequest = requestShift != null;
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final shiftStart = _timeToDouble(
                      displayShift['start_time']?.toString().substring(
                              0, 5) ??
                          displayShift['preferred_start']
                              ?.toString()
                              .substring(0, 5) ??
                          '09:00');
                  ...
                  final startLabel = displayShift['start_time']
                          ?.toString()
                          .substring(0, 5) ??
                      displayShift['preferred_start']
                          ?.toString()
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      return Scaffold(
        appBar: AppBar(
          title: Text(fmt.format(widget.date)),
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
            '${fmt.format(widget.date)} ${_isHoliday ? '（休日）' : '（平日）'}'),
        backgroundColor: Theme.of(context).colorScheme.inversePrimary,
      ),
